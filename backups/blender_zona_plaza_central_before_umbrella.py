...
def rounded_square_base():
    cube("single compact rounded-square plaza tile", (0, 0, 0), (4.18, 4.18, 0.18), M["porcelain"], 0, 0.58)
    cube("thin visible grey underside bevel", (0, 0, -0.075), (4.25, 4.25, 0.07), M["side"], 0, 0.58)
    for radius in (1.28, 1.58):
        ring = torus("thin engraved circular plaza paving guide", (0, 0, 0.135), radius, 0.006, M["line"])
        ring.scale.y = 0.78
    for angle in [math.radians(24), math.radians(156), math.radians(204), math.radians(336)]:
        cube("very subtle curved walkway tick", (math.cos(angle) * 1.74, math.sin(angle) * 1.3, 0.15), (0.28, 0.014, 0.01), M["line"], angle, 0.004)
...
def swoosh():
    ...
    mesh.update()
    obj = bpy.data.objects.new("white nike swoosh on roof", mesh)
...
def tree(x, y, flip=False, scale=1.0):
    cyl("black mini tree trunk", (x, y, 0.3 * scale), 0.032 * scale, 0.32 * scale, M["black"], 12)
    cone("rounded cone tree lower crown", (x, y, 0.54 * scale), 0.13 * scale, 0.27 * scale, M["yellow" if not flip else "white"], 32)
    cone("rounded cone tree upper crown", (x, y, 0.74 * scale), 0.09 * scale, 0.22 * scale, M["white" if not flip else "yellow"], 32)


def round_tree(x, y, scale=1.0):
    cyl("thin trunk for round yellow tree", (x, y, 0.29 * scale), 0.022 * scale, 0.3 * scale, M["pole"], 12)
    sphere("round stylized yellow tree crown", (x, y, 0.56 * scale), 0.145 * scale, M["yellow"], (1, 1, 0.86))
    for angle in (0, math.tau / 3, math.tau * 2 / 3):
        sphere("small lobed round tree crown detail", (x + math.cos(angle) * 0.075 * scale, y + math.sin(angle) * 0.075 * scale, 0.58 * scale), 0.08 * scale, M["yellow"], (1, 1, 0.8))
...
def umbrella(x, y, rot=0, scale=1.0):
    cyl("slender umbrella pole", (x, y, 0.29 * scale), 0.012 * scale, 0.34 * scale, M["pole"], 12)
    canopy = cone("low yellow cafe umbrella canopy", (x, y, 0.5 * scale), 0.17 * scale, 0.115 * scale, M["yellow"], 48)
    canopy.rotation_euler[2] = rot
    for i in range(4):
        angle = rot + i * math.pi / 2
        cube("white umbrella panel rib", (x + math.cos(angle) * 0.05 * scale, y + math.sin(angle) * 0.05 * scale, 0.54 * scale), (0.14 * scale, 0.01 * scale, 0.008 * scale), M["white"], angle, 0.003)
    cyl("tiny round table below umbrella", (x, y, 0.19 * scale), 0.075 * scale, 0.028 * scale, M["yellow"], 28, 0.004)
    for angle in (rot + math.radians(80), rot - math.radians(80)):
        cube("tiny umbrella stool", (x + math.cos(angle) * 0.15 * scale, y + math.sin(angle) * 0.15 * scale, 0.15 * scale), (0.07 * scale, 0.045 * scale, 0.032 * scale), M["white"], angle, 0.006)
