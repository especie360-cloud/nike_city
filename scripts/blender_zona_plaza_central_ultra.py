...
def rounded_square_base():
    rounded_box_mesh("rounded modular toy-city plaza platform top", 4.18, 4.18, 0.12, 0.62, M["porcelain"], 0.03)
    rounded_box_mesh("soft rounded grey platform side wall", 4.22, 4.22, 0.12, 0.64, M["side"], -0.055)
    rounded_box_mesh("thin warm white lower platform lip", 4.08, 4.08, 0.03, 0.58, M["porcelain"], -0.13)
    for radius in (1.28, 1.58):
        ring = torus("thin engraved circular plaza paving guide", (0, 0, 0.105), radius, 0.006, M["line"])
        ring.scale.y = 0.78
    for angle in [math.radians(24), math.radians(156), math.radians(204), math.radians(336)]:
        cube("very subtle curved walkway tick", (math.cos(angle) * 1.74, math.sin(angle) * 1.3, 0.115), (0.28, 0.014, 0.01), M["line"], angle, 0.004)
